...
PUBLIC void do_setquotent(void){
	message m;
	m = mm_in;
	_taskcall(SYSTASK, SYS_SETQUOTENT, &m);
}	
int do_sem_init(void){
	...
	sem_num = mm_in.m1_i1;
	value = mm_in.m1_i2;
	if(sem_num >= MAX_SEMAPHORES || sem_num < 0)
		return EINVAL;
	if(value == -1){
		if(semaphores[sem_num].blocked)
			return EINVAL;
		semaphores[sem_num].ifInit = 0;
		return OK;
	}
	if(semaphores[sem_num].ifInit)
		return EINVAL;
	semaphores[sem_num].value = value;
	semaphores[sem_num].ifInit = 1;
	semaphores[sem_num].blocked = 0;
	semaphores[sem_num].firstBlocked = NULL;
	return OK;
}
int do_sem_down(void){
	int sem_num, callerPid;
	struct pidNode *last, *newNode;
	sem_num = mm_in.m1_i1;
	callerPid = mm_in.m1_i2;
	if(sem_num >= MAX_SEMAPHORES || sem_num < 0)
		return EINVAL;
	if(semaphores[sem_num].value > 0){
		--(semaphores[sem_num].value);
		return OK;
	}
	last = semaphores[sem_num].firstBlocked;
	newNode = (struct pidNode*)alloc_mem(sizeof(struct pidNode));
	(*newNode).processID = callerPid;
	(*newNode).next = 0;
	if(semaphores[sem_num].blocked == 0){
		semaphores[sem_num].firstBlocked = newNode;
		++(semaphores[sem_num].blocked);
		return E_NO_MESSAGE;
	}
	while((*last).next)
		last = (*last).next;
...
int do_sem_up(void){
	...
	struct pidNode *dest;
	message m;
	int sem_num = mm_in.m1_i1;
	if(sem_num >= MAX_SEMAPHORES || sem_num < 0)
		return EINVAL;
	if(semaphores[sem_num].blocked){
		m.m_type = OK;
		dest = semaphores[sem_num].firstBlocked;
		newPid = (*dest).processID;
		semaphores[sem_num].firstBlocked = (*dest).next;
		free_mem(dest, sizeof(struct pidNode));
		--(semaphores[sem_num].blocked);
		mm_in.m1_i1 = newPid;
		newPid = do_getprocnr();
		send(newPid, &m);
...
int do_sem_status(void){
	...
	if(sem_num >= MAX_SEMAPHORES || sem_num < 0)
		return EINVAL;
	return semaphores[sem_num].value;
}
int do_sem_status2(void){
	int sem_num = mm_in.m1_i1;
	if(sem_num >= MAX_SEMAPHORES || sem_num < 0)
		return EINVAL;
	return semaphores[sem_num].blocked;
}
int do_sem_uninit(void){
	int i;
	struct pidNode *prev, *curr;
	for(i = 0; i < MAX_SEMAPHORES; ++i){
		prev = semaphores[i].firstBlocked;
		if(prev){
			while(curr = prev->next){
				free_mem(prev, sizeof(struct pidNode));
				prev = curr;
			}
			free_mem(prev, sizeof(struct pidNode));
		}
